...
const Home = () => {
  ...
  useEffect(() => {
    if (isAppActive) {
      checkMultiple([
        ...
        const locationAlwaysStatus = statuses[PERMISSIONS.IOS.LOCATION_ALWAYS];
        const locationWhenInUseStatus =
          statuses[PERMISSIONS.IOS.LOCATION_WHEN_IN_USE];
        if (locationAlwaysStatus || locationWhenInUseStatus) {
      ...
      if (!isLocationAlwaysAccess) {
        console.log(
          'isLocationAlwaysAccess is',
          isLocationAlwaysAccess,
          'stop location',
        );
        BackgroundGeolocation.stop();
        BackgroundGeolocation.removeAllListeners();
  ...
  // todo Check under what conditions to trigger review popup
  // useEffect(() => {
  //   setTimeout(() => {
        ...
        <View style={styles.mainView} ref={shareViewRef}>
          ...
          {!isShareScreenOn && (
            <>
              ...
              />
              {/* Show the user location privacy setting on the home screen */}
              {isLocationButtonOn && (
